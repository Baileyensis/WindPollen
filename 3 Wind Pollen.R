...
## What if I make Row a random effect?

library("lme4")
me.mod <- glmer(Count ~ Interval + Orchard + Orchard:Interval + Location + 
                  Orchard:Location + (1 | Row) + offset(log(TotalPollen)),
                data = W,
                family = poisson)
summary(me.mod)

# loglik is -386.3 and AIC is 786.5

sum(resid(me.mod, type = "pearson")^2) / (nrow(W) - length(fixef(me.mod) - 1))
# A bit overdispersed... phi = 1.909293

me.negb <- glmer.nb(Count ~ Interval + Orchard + Orchard:Interval + Location + 
                      Orchard:Location + (1 | Row) + offset(log(TotalPollen)), 
                    data = W)
summary(me.negb) # Row as random effect
plot(me.negb)

# loglik is -380.4 and AIC is 776.9

# Now, can we do a rootogram of me.negb1? No.
...
# subsets for Orchard
WHo <- subset(W, Orchard == "HBA")
WCo <- subset(W, Orchard == "CSO")

library(gdata)
# Removing NonCherry from HBA
WHO <- subset(WHo, PollenType != "NonCherry")
WHO <- drop.levels(WHO)
levels(WHO$PollenType)
# Removing NonCherry from CSO
WCO <- subset(WCo, PollenType != "NonCherry")
WCO <- drop.levels(WCO)
levels(WCO$PollenType)

# Removing "Gymno" and "Angio" and "NonCherry" levels from  HBA
WH <- subset(WHo, PollenType != "Other gymnosperm")
WH <- subset(WH, PollenType != "Other angiosperm")
WH <- subset(WH, PollenType != "NonCherry")
WH <- drop.levels(WH)
levels(WH$PollenType)
# Removing "Gymno" and "Angio" and "NonCherry" levels from  CSO
WC <- subset(WCo, PollenType != "Other gymnosperm")
WC <- subset(WC, PollenType != "Other angiosperm")
WC <- subset(WC, PollenType != "NonCherry")
WC <- drop.levels(WC)
levels(WC$PollenType)

# Plotting raw data + boxplots per sample
#HBA 
ggplot(WH, aes(x = Sample, y = Count/TotalPollen, colour = Interval)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.9), size = 1.5) +
  ylim(0,1)
#CSO
ggplot(WC, aes(x = Sample, y = Count/TotalPollen, colour = Interval)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.9), size = 1.5) +
  ylim(0,1)

#plotting raw data + boxplots during AM and PM in separate plots for each orchard
library(ggplot2)
# HBA
ggplot(WHO, aes(x = PollenType, y = Count/TotalPollen, colour = Interval)) +
  geom_boxplot(position = position_dodge(0.9)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.9), size = 1.5) +
  ylim(0,1)
# CSO
ggplot(WCO, aes(x = PollenType, y = Count/TotalPollen, colour = Interval)) +
  geom_boxplot(position = position_dodge(0.9)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.9), size = 1.5) 

## Plotting PollenTypes per Location
# HBA
ggplot(WHO, aes(x = PollenType, y = Count/TotalPollen, colour = Location)) +
  geom_boxplot(position = position_dodge(0.9)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.9), size = 1.5) +
  ylim(0,1) +
  scale_colour_grey(start = 0, end = 0.6, na.value = "red",
                    aesthetics = "colour") 
# CSO
ggplot(WCO, aes(x = PollenType, y = Count/TotalPollen, colour = Location)) +
  geom_boxplot(position = position_dodge(0.9)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.9), size = 1.5) +
  scale_colour_grey(start = 0, end = 0.6, na.value = "red",
                    aesthetics = "colour") 

#############  CALCULATING MEAN POLLEN PROPORTION at HBA
Hmean <- aggregate(WHO$Count/WHO$TotalPollen,
                   by = list(PollenType = WHO$PollenType,
                             Interval = WHO$Interval),
                   FUN = function(x) c(mean = mean(x), variance = mean(x),
                                       n = length(x)))
Hmean <- do.call(data.frame, Hmean)
Hmean
Hmean$sd <- sqrt(Hmean$x.mean)
Hmean$se <- sqrt(Hmean$x.mean / sqrt(Hmean$x.n))
colnames(Hmean) <- c("PollenType", "Interval", "Mean", "Variance", "n", "sd","se")
Hmean

##### errobars and se at HBA
limits.H <- aes(ymax = Hmean$Mean + Hmean$se,
                ymin = Hmean$Mean - Hmean$se)

### means and se plot at HBA
ggplot(Hmean, aes(x=PollenType, y=Mean, colour=Interval)) +
  geom_point(position = position_dodge(0.9), size = 5) +
  geom_errorbar(limits.H, position = position_dodge(0.9), width = 0.25) 

#############  CALCULATING MEAN POLLEN PROPORTION at CSO
Cmean <- aggregate(WCO$Count/WCO$TotalPollen,
                   by = list(PollenType = WCO$PollenType,
                             Interval = WCO$Interval),
                   FUN = function(x) c(mean = mean(x), variance = mean(x),
                                       n = length(x)))
Cmean <- do.call(data.frame, Cmean)
Cmean
Cmean$sd <- sqrt(Cmean$x.mean)
Cmean$se <- sqrt(Cmean$x.mean / sqrt(Cmean$x.n))
colnames(Cmean) <- c("PollenType", "Interval", "Mean", "Variance", "n", "sd","se")
Cmean

##### errobars and se at CSO
limits.C <- aes(ymax = Cmean$Mean + Cmean$se,
                ymin = Cmean$Mean - Cmean$se)

### means and se plot at CSO
ggplot(Cmean, aes(x=PollenType, y=Mean, colour=Interval)) +
  geom_point(position = position_dodge(0.9), size = 5) +
  geom_errorbar(limits.C, position = position_dodge(0.9), width = 0.25) 

library(ggplot2)
ggplot(W, aes(x = PollenType, y = Count/TotalPollen, colour = Orchard)) +
  geom_point(position = position_dodge(0.9), size = 2) 

#HBA
# with counts and poisson distribution
mH <- glm(Count ~ Interval + offset(log(TotalPollen)), data = WH, 
          family = quasipoisson)
summary(mH)
# with proportions and binomial distribution
mH1 <- glm(Count/TotalPollen ~ PollenType + Interval, data = WHO, family = binomial)
summary(mH1)
#CSO
mC <- glm(Count ~ PollenType + Interval + offset(log(TotalPollen)), data = WCO,
          family = poisson)
summary(mC)